...
def correct_scanorama(dataset_list, cell_metadata):
	...
	corrected_dataset.layers['uncorrected'] = raw_dataset.X
...
def subset_by_batches(dataset):
	batch_names = []
	for x in dataset.obs[args.batch_key].values:
		if x not in batch_names:
			batch_names.append(x)
...
if __name__== "__main__":
    ...
    raw_dataset = sc.read(args.input_object)
    read_h5ad(args.input_object)

print(args.output_object)
